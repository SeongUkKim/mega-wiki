...
Mega-Wiki는 Slack에서 발생한 질문을 재사용 가능한 팀 지식으로 전환하는 Spring Boot 백엔드입니다. Slack에서 봇을 멘션하면 질문 흐름이 시작되고, Gemini가 답변 초안을 생성한 뒤, 결과를 Notion 저장소에 기록합니다.
...
즉, 사람이 Notion에서 데이터베이스를 만들고 integration만 연결해두면 보통은 `NOTION_API_TOKEN`만 넣어도 됩니다. 명시적인 data source id를 주면 자동 탐색보다 우선합니다.
...
1. `MegaWiki Pages`, `MegaWiki Questions` 데이터베이스를 만듭니다.
2. 두 데이터베이스에 Notion integration을 연결합니다.
3. 애플리케이션 실행 시 Mega-Wiki가 데이터소스를 자동 탐색하고 스키마를 코드 기준으로 보정합니다.
